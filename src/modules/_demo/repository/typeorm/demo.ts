...
import { DemoRepository } from '../types/implement';

@Injectable()
export class DemoRepositoryTypeorm implements DemoRepository {
  constructor(
  ...
  // CREATE ONE
  // =================================================================
  async create(data: Partial<Demo>): Promise<Demo> {
    const demo = this.repo.create({
      ...data,
      createdAt: new Date(),
      updatedAt: new Date(),
    });
    const savedDemo = await this.repo.save(demo, {
  ...
  async update(
    ...
    const demo = await this.repo.findOneBy({ _id: new ObjectId(_id) });
    if (!demo) return null;
    ...
    this.repo.merge(demo, cleanData);
    demo.updatedAt = new Date();

    const updatedDemo = await this.repo.save(demo, {
      ...options,
    });
    return updatedDemo;
  }
  ...
  // FIND BY ID
  // =================================================================
  async findById(queryDto: input.findById): Promise<Demo | null> {
    const projection = queryDto.select?.length
      ? (queryDto.select as (keyof Demo)[])
      : undefined;

    const demo = await this.repo.findOne({
  ...
  async findAll(queryDto: input.findAll): Promise<{
    ...
    const projection = select?.length ? (select as (keyof Demo)[]) : undefined;
  ...
  async findManyByIds(data: {
    ...
    const demos = await this.repo.find({
      where: {
